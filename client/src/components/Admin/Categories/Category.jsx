...
import {
  ...
  updateSingleCategoryStatus,
} from '../../../redux/Shop/Action/CategoryAction.jsx';
...
const Category = () => {
  ...
  const handleCreateFormSubmit = (e) => {
    ...
    input.image && form_data.append('category-photo', input.image);

    dispatch(createCategory({ data: form_data, setInput, setAdminModal }));
  };
  ...
  const handleStatusBtn = (status, id) => {
    ...
    dispatch(updateSingleCategoryStatus({ data: form_data, id }));
  };
  ...
  return (
    <section className='category-section'>
      ...
      <AdminModal title='Update Brand' adminModal={edit.editModal} setAdminModal={setEdit} setInput={setInput}>
        <Form className='add-modal' onSubmit={handleUpdateFormSubmit}>
          ...
          {input.image ? (
            ...
            <img
              style={{ width: 'auto', height: '200px', textAlign: 'center', display: 'block', marginBottom: '20px' }}
              src={`http://localhost:5050/image/category/${edit.dataBaseImage}`}
              alt=''
            />
          ) : null}
      ...
      <AdminModal title='Add Category' adminModal={adminModal} setAdminModal={setAdminModal} setInput={setInput}>
      ...
      <Container>
        ...
        <Row className='t-title d-flex justify-content-between'>
          ...
          <Col md='6' className='d-flex justify-content-end'>
            <Button onClick={() => setAdminModal(true)}>Add New Category</Button>
          </Col>
        </Row>
                      ...
                      <td>
                        {item.photo ? (
                          <img
                            ...
                            src={`http://localhost:5050/image/category/${item.photo}`}
                            alt=''
                          />
                        ) : (
                          <p
